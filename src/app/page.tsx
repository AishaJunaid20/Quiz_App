...
export default function QuizApp() {
  ...
  if (state.isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-cyan-500 text-foreground">
        <ClipLoader />
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-cyan-400 text-foreground">
      {state.showResults ? (
        // Show results if the quiz is finished
        <div className="bg-card p-8 rounded-lg shadow-md w-full max-w-md bg-cyan-100">
          <h2 className="text-2xl font-bold mb-4 bg-cyan-100">Results</h2>
          <p className="text-lg mb-4">
            You scored {state.score} out of {state.questions.length}
          </p>
          <Button onClick={resetQuiz} className="w-full hover:to-blue-200">
            Try Again
          </Button>
        </div>
      ) : (
